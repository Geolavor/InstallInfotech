...
get_link_git() {
  ...
  print_banner
  printf "${WHITE} 💻 Insira o link da sua instalação InFotech+ que deseja instalar:${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " link_git
}
...
inquiry_options() {
  ...
  print_banner
  printf "${WHITE} 💻 Bem vindo(a) ao Gerenciador InFotech+, Selecione abaixo a proxima ação!${GRAY_LIGHT}"
  printf "\n\n"
